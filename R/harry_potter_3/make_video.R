...
source("R/harry_potter_3/cum_moments.R")

# this keeps numbers from being printed as expo notation
# otherwise 100000 prints as 1e+05 in file name, which 
# keeps it from ordering correctly in the vid
...
walk(1:nrow(t_df), function(i) {
  ...
  p <- cum_df |> 
    ...
    scale_y_continuous(breaks = c(min(these_pts$cumscore), 0, max(these_pts$cumscore)),
                       labels = c("Sadder", "Neutral", "Happier"), limits = c(-300, max_y)) +
    theme_void()
  
  # p
  
  f_ind <- str_pad(pt$i, width = 6, pad = "0")
...
walk(1:nrow(these_pts), function(i) {
  this <- these_pts[i,]
  f_ind <- str_pad(this$i, width = 6, pad = "0")
...
av_encode_video(input = ff_v[order(ff_v)], output = "videos/hp3.mp4")
